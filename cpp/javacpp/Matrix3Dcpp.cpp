...
Matrix3D::Matrix3D () {
  // Changed to unit() because C++ doesn't init all fields to 0 like Java
  unit();
}
...
void Matrix3D::scale(float xf, float yf, float zf) {
  xx *= xf;
  xy *= xf;
  xz *= xf;
  xo *= xf;
...
void Matrix3D::transform(float v[], int tv[], int nvert) {
