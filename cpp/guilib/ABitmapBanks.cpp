...
ABitmapBanks::ABitmapBanks(ADisplay *someDisplay, ADisplayMode *d) : APrivateBitmap(someDisplay,d)
{
  DEBUG_OUT<<"ABitmapBanks::ABitmapBanks()\n";
  init();
...
#endif // ASYS_DOS
  DEBUG_OUT<<"Banked flat bitmap with a window of "<<wsize<<" at "<<wgran<<".\n";
#ifdef ASYS_DOS16
...
void ABitmapBanks::draw(ABitmap *source, unsigned int dx, unsigned int dy)
{
  //DEBUG_OUT<<"Draw on ABitmapBanks!\n";
  ...
  setVGABank(b);
  //DEBUG_OUT<<"(set "<<off<<")\n";
  src=(source->getPlane(0))+(source->getY()*sourceBytesPerLine)+ssx;
  for(row=0;row<drawHeight;row++) {
    memcpy((unsigned char *)(((long)dest)+off),(unsigned char *)src,drawWidth);
    off+=bytesPerLine;
    if(off>wgran) {
      off-=wgran;  b++;
      //DEBUG_OUT<<b<<"\n";
...
void ABitmapBanks::writePixel(unsigned int x, unsigned int y)
{
  if(x>width) x=width;
...
// FIXME: Just a copy of ABitmap::readPixel for now...doesn't handle banks!
unsigned int ABitmapBanks::readPixel(unsigned int x, unsigned int y)
{
  if(eightBits) {
